...
class G2AClientTest(unittest.TestCase):
    def test_upload_posts_json_bytes_direct(self):
        ...
        with mock.patch.object(client._session, "request") as req:
            ...
            self.assertTrue(str(args[1]).endswith("/api/admin/accounts"))
            self.assertIn("Authorization", kwargs["headers"])
            body = kwargs.get("json") or {}
            self.assertEqual(body.get("refresh_token"), "rt")
            self.assertEqual(body.get("name"), "e@x")
    ...
    def test_generate_image_uses_images_api_primary(self):
        client = G2AClient(
            {
                "id": "s1",
                "name": "remote",
    ...
    def test_generate_image_falls_back_to_responses(self):
        client = G2AClient(
            {
                "id": "s1",
                "name": "remote",
        ...
        def side_effect(method, url, **kwargs):
            ...
            if str(url).endswith("/v1/responses"):
                ...
                resp.json.return_value = {
                    ...
                    "model": "grok-4.5",
                    "output": [
                        {"type": "image_generation_call", "status": "completed", "result": png_b64}
                    ],
                }
                return resp
            resp.status_code = 404
