...
def _create_cfd1(extra: dict, proxy: str | None) -> 'BaseMailbox':
    import os
    from core.proxy_env import load_dotenv, resolve_proxy
    ...
    def wait_for_link(
        ...
        while time.time() - start < timeout:
            try:
                ...
                logger.warning("[CF-D1] wait_for_link poll failed: %s", exc)
            time.sleep(3)
        raise TimeoutError(f"等待验证链接超时 ({timeout}s) email={email}")
