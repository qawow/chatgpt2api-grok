...
class RegistrationEngine:
    ...
    def _submit_signup_form(self, did: str, sen_payload: Optional[SentinelPayload]) -> SignupFormResult:
        ...
        try:
            ...
            try:
                ...
                # passwordless_signup also returns email_otp_verification; that is NEW signup OTP,
                # not an already-registered account login.
                is_passwordless_signup = (
                    verification_mode == "passwordless_signup"
                    or signup_mode in {"email_signup", "passwordless_signup"}
                )
                is_existing = (
                    page_type == OPENAI_PAGE_TYPES["EMAIL_OTP_VERIFICATION"]
                    and not is_passwordless_signup
                )
                self._is_passwordless_signup = is_passwordless_signup
                if is_passwordless_signup:
                    self._log(
    ...
    def _send_verification_code(self) -> bool:
        ...
        try:
            ...
            self._log(f"验证码发送状态(POST): {response.status_code}")
            if response.status_code == 200:
                return True
            ...
            self._log(f"验证码发送状态(GET fallback): {response.status_code}")
            if response.status_code != 200:
    ...
    def run(self) -> RegistrationResult:
        ...
        try:
            ...
            # 9. 发送验证码
            # passwordless 与部分新账号并不会在 authorize/continue 时真正发信，
            # 必须显式调用 email-otp/send（探针已验证）。
            if self._is_existing_account:
